...
3. Control Plain 생성
     ...
     + CNI의 구현체를 Flannel을 하기 위해  pod network cidr을 10.244.0.0/16로 설정하여 Control Plain 배포
       ```bash
   ...
   Woker Node는 기존 Control Plain이 설치 과정에 
...
2. Kubeadm, Kubelet, Kubectl 설치
   ...
   + CNI 네트워크 활성화를 위한 네트워크 설정
     ...
     $ sudo sysctl -p
     ```
3. 기존 Kubernetes Cluster에 Join
   
   + Control Plain에 접속하기 위한 명령어 확인.   
     token같은 경우엔 기본적으로 24시간동안만 유효하기 때문에 확인이 필요함.
